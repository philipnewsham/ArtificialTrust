...
public class Timer : MonoBehaviour
{
    ...
    void Update()
    {
        if(m_isCountingDown)
        {
            ...
            {
                timerTextboxes[i].text = string.Format("/TimeRemaining = {0}:{1}", Mathf.Floor((m_maxTime - m_count)/60f), Mathf.Floor((m_maxTime - m_count) % 60f), Mathf.Floor((m_maxTime - m_count) % 10f));
            }
            */
            timerTextboxes[0].text = string.Format("{0}:{1}", Mathf.Floor((m_maxTime - m_count) / 60f), Mathf.Floor((m_maxTime - m_count) % 60f));
            timerTextboxes[1].text = string.Format("/TimeRemaining = {0}:{1}", Mathf.Floor((m_maxTime - m_count) / 60f), Mathf.Floor((m_maxTime - m_count) % 60f));
    ...
    public void RemoveTime(float timeTaken)
    {
        m_count -= timeTaken;
    }
